...
class AsyncConnection(GraphMixin, psycopg.AsyncConnection[Row]):
    ...
    async def load_vertex_frame(
        ...
        No row becomes a Python mapping and no value goes through a dumper: a chunk's property maps
        are written as JSON and the copy stream is built from them. End to end that is 1.5 times as
        fast as handing the same table to :meth:`load_vertices` as mappings -- 1879 ms against 2736
        for two hundred thousand vertices -- and no more, because 1810 ms of either is the copy.
        """
        ...
        # The count is the server's own, taken from the copy's command tag, and it is there only
        # once the copy block has ended -- so the two blocks are nested rather than combined.
