...
Arrow is built a column at a time. The rows are transposed in one step with ``zip(*rows)``, which
also refuses a row that is not as wide as the column names, and each column is handed to one builder
chosen from its type oid and its first value.

A Cypher projection has the jsonb oid whatever it returns, so ``n.name``, ``collect(...)`` and
``properties(n)`` all arrive as 3802. A type oid settles a column only for plain SQL columns and for
the graph types; every other column is settled by its values, and a caller may declare one.

A vertex becomes a struct of its identity, its label and its property map. The map is the JSON the
server sent, carried across as text without being decoded. An edge adds ``start`` and ``end``, and a
path is a struct of a list of vertices and a list of edges. An identity is a ``uint64``: the label id
in the top sixteen bits, the serial id in the low forty-eight.

A dense vector arrives as a header and its numbers as big-endian single-precision floats, which is
what ``FixedSizeList<float32>`` holds. Every row's bytes go into one ``array('f')``, which is
byte-swapped once for the column and handed to Arrow as a buffer. That array and the Arrow column
over it share memory, as do the Arrow column and a polars column made from it for numbers and
vectors. polars holds strings its own way, and pandas copies every column whatever dtypes it is
given.

Loading the other way goes through binary ``COPY``. A chunk's property maps are written as JSON and
the copy stream built from them, so no row becomes a Python mapping and no value goes through a
dumper.

Each backend is imported inside the function that uses it, so a program that does not export loads
none of them.
"""
...
def _json_array(payloads: Sequence[bytes | None]) -> Any:
    ...
    Built as binary and cast, which validates the UTF-8.
    """
    import pyarrow
...
def _sql_types(pyarrow: Any) -> dict[int, Any]:
    ...
    Only the ones a value cannot settle. jsonb is absent, since every Cypher expression carries that
    oid whatever it holds; numeric is absent, since a Decimal settles to the decimal type that fits.
    """
    return {
        16: pyarrow.bool_(),
        17: pyarrow.binary(),
...
def _column_for(
    ...
    The graph types are settled by their oid, everything else by the first value that is not null.
    A column of nothing but nulls and no declared type becomes a null column.

    A map, or a list of maps, becomes JSON text. A list of anything else becomes an Arrow list
    column.
    """
    if oid == GRAPHID_OID:
...
class Plan:
    ...
    Settled by the first chunk and held thereafter, so every chunk carries the same schema and the
    chunks concatenate. A chunk that does not fit the settled schema raises.
    """
...
def _source(
    ...
    A cursor carries the column types and hands out rows a chunk at a time; a list of rows carries
    neither.
    """
...
def to_arrow(
    ...
    *,
    ...
    ``size`` of ``None`` builds one batch per chunk the source hands over, which for a result already
    in memory is one batch, so an undeclared column's type is settled by all of its values. Giving a
    size chunks it, and the first chunk settles the type.
    """
    import pyarrow
...
def to_polars(
    ...
    *,
    ...
    Built from the Arrow table. polars takes over its number and vector columns without copying, and
    copies a string column.
    """
    import polars
...
def _payloads(batch: Any) -> list[bytes]:
    ...
    polars writes the JSON where it is installed, and msgspec otherwise. Both write the same bytes.
    """
    import pyarrow
