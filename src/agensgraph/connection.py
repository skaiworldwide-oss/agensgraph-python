...
class Connection(GraphMixin, psycopg.Connection[Row]):
    ...
    def deadline(
        ...
        **It costs two statements, so put it where many statements are inside it.** Over a loopback
        ...
        if limit is None:
            yield held
            return
        self.execute(f"set statement_timeout = {limit}")
    ...
    def read_only_transaction(self, *, allow_server_programs: bool = False) -> Generator[None]:
        ...
        **What that costs, since it is not nothing.** psycopg gives up its prepared statements on
        ...
        with self.transaction() as block:
    ...
    def upsert_vertices(
        ...
        **A key with no uniqueness behind it is refused.** Two writers merging on the same key
        ...
        name = self._graph_of(graph)
        material = list(rows)
        indexed = self._key_is_unique(label, key, graph=name)
    ...
    def identity_map(
        ...
        **Name the keys you need, if you know them.** With ``keys`` given, only those are looked
        ...
        """
        name = self._graph_of(graph)
        if keys is not None and self._key_is_unique(label, key, graph=name):
            return self._identity_of_keys(label, key, keys, graph=name)
        column = self._promoted_key_column(label, key, graph=name)
        if column is not None:
            rows = self._fetch(promoted_identity_map_statement(name, label, key), ())
        else:
            rows = self._fetch(identity_map_statement(name, label), (key,))
        return build_identity_map(rows, label=label, key=key)

    def _promoted_key_column(self, label: str, key: str, *, graph: str) -> str | None:
        ...
        different element.
        """
        if not self.can_promote_properties():
            return None
        for declared in self.declared_properties(label, graph=graph):
            if declared.name == key:
                return declared.type if declared.type in PROMOTED_KEY_TYPES else None
        return None

    def _identity_of_keys(
    ...
    def can_promote_properties(self) -> bool:
        ...
        if held is None:
            rows = self._fetch(PROMOTION_CATALOG_QUERY, ())
            held = bool(rows[0][0])
    ...
    def describe(
        ...
        **Nothing is installed.** Three of the packages this replaces create a plpgsql function in
        ...
        """
        name = self._graph_of(graph)
        if refresh and (not self.meta_is_current(graph=name)):
            self._run(GATHER_META)
        labels = self.labels(graph=name)
        counts = self.element_counts(graph=name)
        triples = tuple(
            (
                Triple(start, edge, end, seen)
                for start, edge, end, seen in self._fetch(TRIPLES_QUERY, (name,))
            )
        )
        declared: dict[str, dict[str, str]] = {}
        if self.can_promote_properties():
            for each in self.declared_properties(graph=name):
                declared.setdefault(each.label, {})[each.name] = each.type
        wanted = [label.name for label in labels if not label.is_builtin]
        sampled = self.pipeline_query(
        ...
        return GraphDescription(
            ...
            triples=triples,
            counts=counts,
            meta_gathered=self.meta_is_current(graph=name),
        )

    def meta_is_current(self, *, graph: str | None = None) -> bool:
        ...
        """
        name = self._graph_of(graph)
        if self._has_meta_flag():
            rows = self._fetch(META_VALID_QUERY, (name,))
            return bool(rows) and bool(rows[0][0])
        triples = self._fetch(TRIPLES_QUERY, (name,))
        if triples:
            return True
        counts = self.element_counts(graph=name)
        labels = self.labels(graph=name)
    ...
    def vector_indexes(
        ...
        """
        name = self._graph_of(graph)
        found = self.indexes(label, graph=name)
        types = {(d.label, d.name): d.type for d in self.declared_properties(graph=name)}
    ...
    def ensure_indexes(
        ...
        *,
        ...
        if cast:
            declared = {(d.label, d.name) for d in self.declared_properties(graph=name)}
            promoted = sorted(cast & declared)
            if promoted:
                raise ValueError(
                    f"{promoted} have columns of their own, and an index over a cast of the property is never used by a search on the column. Leave the cast out."
                )
    ...
    def ensure_labels(
        ...
        *,
        ...
        if any(want.properties for want in desired):
            ...
            declared = self.declared_properties(graph=name)
        statements = reconcile_labels(desired, self.labels(graph=name), declared)
        if dry_run:
            return statements
        for statement in statements:
            self._run(statement)
